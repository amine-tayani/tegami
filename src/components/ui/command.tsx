...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent className="overflow-hidden p-0 [&>button:last-child]:hidden sm:min-w-[600px] md:min-w-[750px] lg:min-w-[900px] ">
...
function CommandInput({
  ...
  return (
    <div
      className="flex items-center py-2 px-5 bg-muted/70 rounded-t-xl"
      cmdk-input-wrapper=""
    >
      <SearchIcon size={20} className="text-foreground me-3" />
      <CommandPrimitive.Input
        data-slot="command-input-wrapper"
        className={cn(
          "placeholder:text-muted-foreground/60 flex h-10 w-full rounded-xl py-3 text-lg outline-hidden disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
        {...props}
      />
...
function CommandShortcut({
  ...
  return (
    <kbd
      ...
      className={cn(
        "bg-background text-muted-foreground/70 ms-auto -me-1 inline-flex h-5 max-h-full items-center rounded-xl px-1 font-[inherit] text-[0.625rem] font-medium",
        className
      )}
      {...props}
    />
